...
# Old data - with slightly incorrect tray set up
# However, basic results remain the same

lego_search <- read_csv('data/lego_search_old.csv') %>%
  pivot_longer(c(colour, shape, conjunction), names_to = "condition", values_to = "rt") %>%
  rename(participant = "Participant") %>%
  mutate(logrt = log(rt),
         condition = as_factor(condition),
...
lego_search %>% modelr::data_grid(condition) %>% 
  ...
  scale_color_brewer(palette = "Purples",direction  = -1) + 
  theme(legend.position = "none") + 
  ggtitle("(i) Distributions & Model Predictions for Task 1A") 
...
bs %>% select(colour, conjunction, shape) %>%
  ...
  geom_density(alpha = 0.5)  + 
  theme(legend.position = "none") +
  ggtitle("(ii) Posterior Densities for Task 1A") 
